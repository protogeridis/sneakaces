...
function woo_wc_search_form( $form ) {
    ...
    <input type="search" results=5 autosave="'. esc_url(home_url( '/' )) .'" class="input-text" placeholder="'. esc_attr__( 'Search', 'woothemes' ) .'" value="' . get_search_query() . '" name="s" id="s" />
    <input type="hidden" class="button" id="searchsubmit" value="'. esc_attr__( 'Search', 'woothemes' ) .'" />
...
function mystile_featured_products() {
	...
	if (class_exists('woocommerce') && $woo_options[ 'woo_homepage_featured_products' ] == "true" ) {
		echo '<div class="featured-products"><h2 class="underbold">'.__('Featured Products', 'woothemes').'</h2>';
